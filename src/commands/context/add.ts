...
import { addContext } from '../../utils/Context';
...
export default class ContextAdd extends Command {
  ...
  async run() {
    ...
    this.log(`Added context "${contextName}".\n\nYou can set it as your current context: otto config context use ${contextName}\nYou can use this context when needed by passing ${contextName} as a parameter: otto validate ${contextName}`);
  }
}
